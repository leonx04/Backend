...
//    }
    ...
    public ProductVariant update(ProductVariant productVariant) {
        if (!variantRepo.existsById(productVariant.getId())) {
            throw new ResourceNotFoundException("Update failed: ProductVariant not found with ID: " + productVariant.getId());
        }
    ...
    @Override
    public List<ProductVariant> findByPromotionId(Integer promotionId) {
        return variantRepo.findByPromotionId(promotionId);
    }
